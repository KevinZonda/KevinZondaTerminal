...
.PHONY: help deps install restore web build run run-server auth-init auth-add auth-verify test test-desktop test-server test-server-auth test-auth format audit publish publish-desktop publish-server clean

help:
	@echo "Available targets:"
	@echo "  make deps      - restore NuGet and pnpm dependencies"
	@echo "  make install   - install zt.exe and kterm-server.exe"
