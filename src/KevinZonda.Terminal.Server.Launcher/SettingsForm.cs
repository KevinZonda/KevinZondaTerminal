...
internal sealed class SettingsForm : Form
{
    ...
    private readonly TextBox _icpRegistration = new() { Dock = DockStyle.Fill, MaxLength = 128 };
        ...
        string configurationPath)
    {
        ...
        _icpRegistration.Text = configuration.Server.IcpRegistration ?? string.Empty;
        _icpRegistration.PlaceholderText = "Hidden when empty, e.g. 沪ICP备12345678号-1";
        ...
        AddSetting(layout, "ICP registration", _icpRegistration, 4);
        AddSetting(layout, "Working directory", workingDirectoryPanel, 5);
        AddSetting(layout, "Runtime retention (minutes)", _runtimeRetention, 6);
        AddSetting(layout, "Public certificate", publicCertificatePanel, 7);
        AddSetting(layout, "Private key", privateKeyPanel, 8);
        AddSetting(layout, "Certificate tools", certificateTools, 9);
        ...
        layout.SetColumnSpan(layout.GetControlFromPosition(0, 10)!, 2);
        layout.Controls.Add(help, 0, 11);
        layout.SetColumnSpan(help, 2);
        layout.Controls.Add(_additionalArguments, 0, 12);
        layout.SetColumnSpan(_additionalArguments, 2);
        layout.Controls.Add(pathLabel, 0, 13);
        ...
        try
        {
            ...
            Configuration = new LauncherConfiguration
            {
                ...
                Server = new LauncherServerConfiguration
                {
                    ...
                    IcpRegistration = string.IsNullOrWhiteSpace(_icpRegistration.Text)
                        ? null
                        : _icpRegistration.Text,
